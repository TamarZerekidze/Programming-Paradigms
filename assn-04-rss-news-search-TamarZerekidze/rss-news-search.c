...
static void BuildIndices(const char *feedsFileName,hashset *stopWords, hashset *eachWord);
static void ProcessFeed(const char *remoteDocumentName,hashset *stopWords, hashset *eachWord);
static void PullAllNewsItems(FILE *dataStream,hashset *stopWords, hashset *eachWord);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *eachWord);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL,hashset *stopWords, hashset *eachWord);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset *stopWords, hashset *eachWord);
static void QueryIndices(hashset *stopWords, hashset *eachWord);
static void ProcessResponse(const char *word, hashset *stopWords, hashset *eachWord);
...
static const char *const kDefaultStopWords = "data/stop-words.txt";
static const char *const kFilePrefix = "file://";
static const char *const kTextDelimiters =
    " \t\n\r\b!@$%^*()_+={[}]|\\'\":;/?.>,<~`";
static const char *const kNewLineDelimiters = "\r\n";
static const int nStopBuckets = 1009;
static const int nBuckets = 10007;
static const int alloc_len = 100;
static const int article_num = 10;
...
int StrCompare(const void* el1, const void* el2){
  char* str1 = *(char**)el1;
  char* str2 = *(char**)el2;
  return strcasecmp(str1,str2);
}

static void StrFree(void* el){
  free(*(char**)el);
}

static void VectorFreeFn(void* ad){
  articleInfo* artc = (articleInfo*)ad;
  free(artc->articleURL);
  free(artc->articleTitle);
}

static int ArticleCompareFn(const void* elemAddr1, const void* elemAddr2){
  articleInfo* first = (articleInfo*)elemAddr1;
  articleInfo* second = (articleInfo*)elemAddr2;
  const char* name1 = first->articleTitle;
  const char* url1 = first->articleURL;
  const char* name2 = second->articleTitle;
  const char* url2 = second->articleURL;

  if(strcasecmp(name1,name2) == 0 || strcasecmp(url1,url2) == 0) return 0;
  return strcasecmp(name1,name2);
}

static int VectorNumCompareFn(const void* el1, const void* el2){
  articleInfo* ar1 = (articleInfo*)el1;
  int n1 = ar1->occurrences;
  articleInfo* ar2 = (articleInfo*)el2;
  int n2 = ar2->occurrences;
  return (n2-n1);
}

static int WordHashFn(const void* elem, int numBuckets){
  char* s = ((currKey*)elem)->key;
  int i;
  unsigned long hashcode = 0;
  for (i = 0; i < strlen(s); i++)  
  hashcode = hashcode * kHashMultiplier + tolower(s[i]);  
...
static int WordCompareFn(const void* el1, const void* el2){
currKey* k1 = (currKey*)el1;
char* w1 = k1->key;
currKey* k2 = (currKey*)el2;
char* w2 = k2->key;
return strcasecmp(w1,w2);
}

static void WordFreeFn(void* e){
free((*(currKey*)e).key);
VectorDispose(&((*(currKey*)e).v));

}



void StopWordsHashset(hashset* stopWords,const char* OpenFile){
  FILE* infile;
  streamtokenizer stn;
  char buffer[1024];
  ...
  assert(infile != NULL);
  HashSetNew(stopWords,sizeof(char*),nStopBuckets,StringHash,StrCompare,StrFree);

  STNew (&stn,infile,kNewLineDelimiters,true);
  while(STNextToken(&stn,buffer,sizeof(buffer))){
...
int main(int argc, char **argv) {
  ...
  hashset stopWords,eachWord;
  
  StopWordsHashset(&stopWords,kDefaultStopWords);
  HashSetNew(&eachWord,sizeof(currKey),nBuckets,WordHashFn,WordCompareFn,WordFreeFn);
  //HashSetNew(&seenArticles,sizeof(articleInfo),nBuckets,ArticleHashFn,ArticleCompareFn,ArticleFreeFn);

  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1],&stopWords,&eachWord);
  QueryIndices(&stopWords,&eachWord);
  
  HashSetDispose(&eachWord);
...
static void BuildIndices(const char *feedsFileName,hashset *stopWords, hashset *eachWord) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName,stopWords,eachWord);
  }
...
static void ProcessFeedFromFile(char *fileName,hashset *stopWords, hashset *eachWord) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName,stopWords,eachWord);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName,hashset *stopWords, hashset *eachWord) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix),stopWords,eachWord);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed,stopWords,eachWord);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset *stopWords, hashset *eachWord) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, stopWords,eachWord);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *eachWord) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL,stopWords,eachWord);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL,stopWords,eachWord);
  STDispose(&st);
  fclose(tmpDoc);
}

static void IndexWord(char* curr,const char* articleTitle,const char* articleURL,hashset* eachWord){
  currKey newKey;
  char* w = strdup(curr);
  newKey.key = w;
  VectorNew(&(newKey.v),sizeof(articleInfo),VectorFreeFn,alloc_len);
  void* elem = HashSetLookup(eachWord,&curr); // idk if lookup actually can do this

  articleInfo newArticle;
  newArticle.articleTitle = strdup(articleTitle);
  newArticle.articleURL = strdup(articleURL);
  newArticle.occurrences = 1;  
  if(!elem){
    VectorAppend(&newKey.v,&newArticle);
    HashSetEnter(eachWord,&newKey);
  } else {
    free(w);
    currKey* elemAddress = (currKey*)elem;
    vector* l = &(elemAddress->v);
    int dx = VectorSearch(l,&newArticle,ArticleCompareFn,0,false);
    if(dx != -1){
      articleInfo* currArticle = (articleInfo*)VectorNth(l,dx);
      (currArticle->occurrences)++;
      free(newArticle.articleTitle);
      free(newArticle.articleURL);
    }else {
      VectorAppend(l,&newArticle);
    }

    VectorDispose(&newKey.v);
    //article disposal too? cause i used strdup so idk, but i dispose them elswhere(?)
  }
  //free(w);
}
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        char* curr = word;
        if(!HashSetLookup(stopWords,&curr)){
          IndexWord(curr,articleTitle,articleURL,eachWord);
        }
        numWords++;
...
static void QueryIndices(hashset *stopWords, hashset *eachWord) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response,stopWords,eachWord);
  }
}
...
static void ProcessResponse(const char *word,hashset *stopWords, hashset *eachWord) {
  if (WordIsWellFormed(word)) {
    ...
    if(!HashSetLookup(stopWords,&word)){
      currKey nwKey;
      char* tmp = strdup(word);
      nwKey.key = tmp;
      currKey* address = (currKey*)HashSetLookup(eachWord,&tmp);
      if(!address){
        printf("None of today's news articles contain the word \"%s\".\n", word);
        return;
      }
      vector* vct = &(address->v);
      VectorSort(vct,VectorNumCompareFn);

      int ArticleNum = VectorLength(vct);
      if(ArticleNum > article_num){
        ArticleNum = article_num;
      }
      for(int i = 0; i < ArticleNum; i++){
        articleInfo* elem = (articleInfo*)VectorNth(vct,i);
        bool b = (elem->occurrences <= 1);
        if(!b){
         printf("%d.) \"%s\" [search term occurs %d times]\n\"%s\"\n", i + 1,elem -> articleTitle, elem -> occurrences, elem -> articleURL);
        } else {
         printf("%d.) \"%s\" [search term occurs 1 time]\n\"%s\"\n", i + 1,elem -> articleTitle, elem -> articleURL); 
        }
      }
      free(tmp);
  } else {
