...
(define (intersection-points ls)
      ...
      (append
       (apply append 
        (map (lambda (second) 
        (intersect (car ls) second ))
         (cdr ls))) 
       (intersection-points (cdr ls)))))


(define (distance-product elem ls)
 (let ((newls (remove elem ls)))
 (apply * (map
 (lambda (each) (dist elem each))
 newls)))
)

(define (rate-points ls)
(map (lambda (element) (list (distance-product element ls) element
 ))ls))
...
(prefix-of-list
(map (lambda (each) (cadr each)) 
(sort-points (rate-points ls)))
...
(let 
((avpt 
(make-pt (/ (apply + (map car ls)) (length ls)) 
        (/ (apply + (map cadr ls)) (length ls)))
        ))
(list (distance-product avpt ls) avpt))
)

(define (best-estimate circle)
 (average-point (clumped-points (intersection-points circle))))

 (define (where-am-i ds stars)
 (sort-points
 (map best-estimate (all-guesses ds stars))))
