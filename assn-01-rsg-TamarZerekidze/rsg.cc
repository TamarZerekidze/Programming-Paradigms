...
static void generateSentences(const string& key, map<string, Definition>& grammar){
    if (grammar.find(key) == grammar.end()){
        exit(EXIT_FAILURE);
    }

    Production pr = grammar[key].getRandomProduction();
    for(Production::iterator curr = pr.begin(); curr != pr.end(); curr++){
        string word = *curr;
        if(word[0] != '<')
          cout << word << " ";
        else 
          generateSentences(word, grammar);
    }
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < 3; i++){
    cout << "Version #" << (i+1) << ": -----------------------\n" ;
    generateSentences(string("<start>"), grammar);
    cout << endl; //cout << endl;
}
  return 0;
}
