...
def findOptimalAlignment(strand1, strand2, ch):
	# if one of the two strands is empty, then there is only
	# one possible alignment, and of course it's optimal
	if len(strand1) == 0: return [' '*len(strand2), strand2, len(strand2) * -2]
	if len(strand2) == 0: return [strand1, ' '*len(strand1), len(strand1) * -2]
	...
	# they actually match.
	bestWith = findOptimalAlignment(strand1[1:], strand2[1:],ch)
	if strand1[0] == strand2[0]:
		ch[key] = [strand1[0] + bestWith[0], strand2[0] + bestWith[1], bestWith[2] + 1]
		return ch[key]

	best = [strand1[0] + bestWith[0], strand2[0] + bestWith[1], bestWith[2] - 1]

	# It's possible that the leading base of strand1 best
	# matches not the leading base of strand2, but the one after it.
	bestWithout = findOptimalAlignment(strand1, strand2[1:], ch)
	bestWithout[2] -= 2 # penalize for insertion of space
	if bestWithout[2] > best[2]:
		bestWithout[0] = ' ' + bestWithout[0]
		bestWithout[1] = strand2[0] + bestWithout[1]
		best = bestWithout

	# opposite scenario
	bestWithout = findOptimalAlignment(strand1[1:], strand2, ch)
	bestWithout[2] -= 2 # penalize for insertion of space
	if bestWithout[2] > best[2]:
		bestWithout[1] = ' ' + bestWithout[1]
		bestWithout[0] = strand1[0] + bestWithout[0]
		best = bestWithout

	ch[key] = best
	return best

def scores(strand1, strand2):
	marks = ['', '']
...
def main():
	...
	while (True):
		...
		alignment = findOptimalAlignment(strand1, strand2 , ch)
		marks = scores(alignment[0],alignment[1])
		printAlignment(alignment[2])
		test(alignment[2], marks[1], marks[0], alignment[0], alignment[1])
