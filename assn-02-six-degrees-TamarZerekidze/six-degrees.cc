...
void generateShortestPath(const string& source, const string& target, const imdb& db){
  ...
  while(!partialPaths.empty() && partialPaths.front().getLength() <= 5) {
    ...
    if(db.getCredits(lastActor, lastActorFilms)){
      for(long unsigned int i = 0; i < lastActorFilms.size(); i++){
        film curr = lastActorFilms[i];
        if(previouslySeenFilms.find(curr) == previouslySeenFilms.end()){
          previouslySeenFilms.insert(curr);
          
          vector<string> cast;
          if(db.getCast(curr,cast)){
            for (long unsigned int j = 0; j < cast.size(); j++){
              string currActor = cast[j];
              if(previouslySeenActors.find(currActor) == previouslySeenActors.end()){
                previouslySeenActors.insert(currActor);

                path clonedPath = currPath;

                clonedPath.addConnection(curr,currActor);
